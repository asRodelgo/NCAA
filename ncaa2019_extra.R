...
team_stats_cluster3 <- gather(team_stats_cluster2, opp_cluster, net_rtg, -contains("team")) %>%
  mutate(opp_cluster = as.numeric(opp_cluster), net_rtg = as.numeric(str_replace_na(net_rtg, 0)))
#
...
for (i in 1:(length(selectedTeams)-1)) {
  for (j in (i+1):length(selectedTeams)) {
    ...
    if (nrow(thisTeamA)*nrow(thisTeamB) > 0){
      muA <- thisTeamA$Off_rating[1] + thisTeamB$Def_rating[1] - mu + thisTeamA$tourneyPts[1] - thisTeamB$tourneyPts[1]
      muB <- thisTeamB$Off_rating[1] + thisTeamA$Def_rating[1] - mu + thisTeamB$tourneyPts[1] - thisTeamA$tourneyPts[1]
...
submission2_prep <- left_join(submission2_prep, team_stats_cluster3, by = c("team_A"="team_id","cluster_A"="team_cluster","cluster_B"="opp_cluster")) %>%
  left_join(team_stats_cluster3, by = c("team_B"="team_id","cluster_B"="team_cluster","cluster_A"="opp_cluster")) %>%
  mutate(net_rtg_diff = net_rtg.x - net_rtg.y) %>%
  select(-contains(".")) %>%
  mutate(prob_clst = 1-pnorm(0,muA-muB+net_rtg_diff/10,sqrt(2)*sigma),
         prob_final_clst = prob_clst - (rank_diff + seed_diff)/100) %>%
  mutate(prob_final_clst = ifelse(prob_final_clst > 1, 1, ifelse(prob_final_clst < 0, 0, prob_final_clst))) %>%
  ungroup() %>%
  distinct(teamA, teamB, .keep_all = TRUE)

submission2_onlyProbs <- select(submission2_prep, team_A, team_B, contains("prob")) %>%
...
submission3clst <- mutate(submission2clst, Pred = ifelse(Pred >=.95,.999,ifelse(Pred<=.05,0.001,Pred))) %>%
  #select(Id,Pred,Team_Name_A = Team_Name.x, Team_Name_B = Team_Name.y) %>%
  select(Id,Pred) %>%
  arrange(Id)
...
# compare submissions
compare_subm <- inner_join(submission, select(submission2, Id, Pred2 = Pred), by = "Id") %>%
  inner_join(select(submission3, Id, Pred3 = Pred), by = "Id") %>%
  inner_join(select(submission2clst, Id, Pred2clst = Pred), by = "Id") %>%
  inner_join(select(submission3clst, Id, Pred3clst = Pred), by = "Id")

ensemble_subm <- gather(compare_subm, Prediction, Value, -Id) %>%
  group_by(Id) %>%
